      ...
      <div className="sm:hidden block  bg-white mb-[5%] ">
        <div className="bg-[#218686] bg-blend-multiply flex items-center justify-center  rounded-bl-[80%_100px] rounded-br-[80%_100px] w-full h-[45vh] bg-cover bg-center bg-[url('https://images.pexels.com/photos/1571457/pexels-photo-1571457.jpeg')]">
          <div className="flex text-white items-center text-center gap-4 py-[10%] px-[8%] flex-col">
            ...
            <p className="font-bold text-[180%] font-primary text-white">From Design to Reality</p>
        ...
        <div>
          <div className="flex flex-col text-center justify-center items-center px-[8%] my-20">
            <p className="font-semibold font-primary text-[#305658] text-[110%]">BUILT TO LAST</p>
                ...
                {project.status === "Upcoming" ? (
                  <div className="w-full h-[50vh] bg-[#218686]/60 rounded-4xl relative">
                    <Image
                      ...
                      className="object-cover  h-full w-full mix-blend-multiply  rounded-4xl"
                    />
                    <div className="absolute top-[45%] left-[25%] font-semibold font-sencondary text-black text-[150%]"> Coming Soon.....</div>
                  </div>
                ) : (
                  <>
                    {/* Completed / other projects */}
                    ...
                    <div className="flex flex-col h-full gap-5 px-5">
                      <p className="font-bold  font-primary text-[#063231] text-[150%]">{project.title}</p>
                      <p className="font-sencondary text-[#305658] text-[110%] ">{project.desc}</p>
                      {project.highlights.length > 0 && (
                        <>
                          <p className="font-semibold text-[140%] text-[#063231] font-primary">Highlights</p>
                          {project.highlights.map((item, i) => (
                            <div key={i} className="flex gap-2">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                              ...
                              <p className="font-sencondary text-[#305658]">{item}</p>
                            </div>
                          ))}
                        </>
                      )}
                    ...
                    <hr />
                    <p className="px-5 pb-3 text-gray-800 text-[100%] ">Type: <span className="text-[#063231] font-semibold">{project.type}</span></p>
                  </>
                )}
              </div>
            ))}
      ...
      <div className="lg:hidden md:block hidden bg-white mb-[5%]  ">
        <div className="bg-[#218686] bg-blend-multiply flex items-center justify-center  rounded-bl-[80%_100px] rounded-br-[80%_100px] w-full h-[45vh] bg-cover bg-center bg-[url('https://images.pexels.com/photos/1571457/pexels-photo-1571457.jpeg')]">
          <div className="flex text-white items-center text-center gap-4 py-[10%] px-[8%] flex-col">
            ...
            <p className="font-bold text-[180%] font-primary text-white">From Design to Reality</p>
            <p className="font-sencondary text-[100%] text-w'">
              At Sri Velan Constructions, we turn every idea into a strong foundation of trust, quality, and craftsmanship. Explore our proud works — where each project speaks of dedication, precision, and the promise to last for generations.
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-10  justify-center pb-10">
            {["All", "Completed", "Upcoming"].map((cat) => (
              <p
                key={cat}
                onClick={() => setFilter(cat)}
                className={`cursor-pointer font-bold mx-[5%] text-[#305658] text-[120%] items-center text-center py-4  tracking-wider shadow-xl rounded-xl transition-colors 
        ${filter === cat
                    ? "font-bold font-primary tracking-wider bg-[#063231] text-[#cbff54]"
                    : "hover:bg-[#0f4544] font-primary bg-gray-200 hover:text-[#cbff54]"
                  }`}
                ...
                {project.status === "Upcoming" ? (
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-5 px-5">
                      <p className="font-bold text-[200%]  text-[#063231] font-primary">{project.title}</p>
                      <p className="font-sencondary text-[#305658] text-[120%] ">{project.desc}</p>
                      {project.highlights.length > 0 && (
                        <>
                          <p className="font-semibold text-[#063231]  font-primary text-[150%] ">Highlights</p>
                          {project.highlights.map((item, i) => (
                            <div key={i} className="flex gap-2 text-[120%] ">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="24"
                                height="24"
